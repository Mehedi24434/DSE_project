...
def Dse_rolling_ticker_scrapper():
    ...
    return df
def ready_influx (write = False, query = False, bucket = 'bucx'):
...
def data_writter():
    ...
    for index, row in x.iterrows():
        data = Point("stocks2").tag("ticker", row["Company"]).field("Price", row["Price"]).field("Change", row["Change"]).tag("time-zone", str(row["DateTime"]))
        ListA.append(data)
...
def data_reader():
    ...
    query = f'from(bucket:"{bucket}") \
         |> range(start: 0) \
         |> filter(fn: (r) => r._measurement == "stocks2")'
    li = []
...
status = Time_checker(Market_Status=True)
while status == "Open" :
    data_writter()
    time.sleep(60)
